...
while(1):
        ...
        for cnt in contours:
            if (cv2.contourArea(cnt)>500)&(cv2.contourArea(cnt)<50000): #&(cv2.contourArea(cnt)<2500):
                ...
                if checkHisto:
                    ...
                    isBee = Utils2.checkColors(frame, xCentr, yCentr, width, height)
                if isBee:
                    M = cv2.moments(cnt)
                    cx = (M["m10"]/M["m00"])
                    cy = (M["m01"]/M["m00"])
    ...
    if len(newBeeTable)*len(beeTable)>0:
        compareBeesTable = Utils2.calculateDistance(beeTable,newBeeTable)
    if len(newBeeTable)*len(beeTable)>0:# !=[]) and (beeTable!=[]):
        for testIndex in range(len(beeTable)):
            indice = np.argmin(compareBeesTable)
            if(np.min(compareBeesTable>1000)):
                break
            row = int(indice/len(newBeeTable))
            bee = beeTable[row]
            col = indice%len(newBeeTable)
            newBee = newBeeTable[col]
            bee.state = 2
            bee.counter = 0
            bee.update = True
            ...
            if oldY>coorYCrossingline and bee.positionY<coorYCrossingline:
                exit +=1
            beeTable[row]=bee
            compareBeesTable[row,:] = 10000
            compareBeesTable[:, col] = 10000
            newBee.update = True
            newBeeTable[col]=newBee
    ...
    for bee in newBeeTable:
        if((bee.positionX<1920/25*3)|(bee.positionY>1080/18*2)|(bee.positionX>1920/25*22)):
            ...
            bee.state = 0
            beeTable.append(bee)
            if Utils2.visible(bee,beeTable):
                ellipse = cv2.fitEllipse(bee.cnt)
                cv2.ellipse(frame, ellipse, (0,0,255),2)
                cv2.putText(frame,str(bee.id),(int(bee.positionX),int(bee.positionY)), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2,cv2.LINE_AA)
